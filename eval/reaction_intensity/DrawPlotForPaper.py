...
SBOT_TSR = np.array(SBOT_cases.iloc[:, 13:23])
SBOT_TSR_r = np.divide(SBOT_TSR[:, 1:], np.repeat(np.expand_dims(SBOT_TSR[:, 0]+1405, axis=1), 9, axis=1))

HGSOC_cases = df.loc[~df["case_id"].str.contains("OCMC", case=False)]

HGSOC_TSR = np.array(HGSOC_cases.iloc[:, 13:23])
HGSOC_TSR_r = np.divide(HGSOC_TSR[:, 1:], np.repeat(np.expand_dims(HGSOC_TSR[:, 0], axis=1), 9, axis=1))
