...
// directory save the cell measurements (with cell labels)
def data_dir = "\\\\anonymized_dir\\result_analysis\\roi_cell_classifications"
def server = QP.getCurrentImageData().getServer()
F = new File(server.getMetadata().getName())
print(F.getName().lastIndexOf('.'))
case_id = F.getName().take(F.getName().lastIndexOf('.'))
print(sprintf("Processing case %s", case_id))
txt_fn = data_dir + File.separator + case_id + "_cell_features_prediction.txt"
fh = new File(txt_fn)
