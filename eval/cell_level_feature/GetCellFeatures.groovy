...
//     print(grp)
    ...
    for (g in grp){
        ...
        def currentclasses = project.getPathClasses()
        def pathClass = "ROI"
        def mynewclass = PathClassFactory.getPathClass(pathClass)
        mynewclass.setColor(ColorTools.makeRGB(0,127,255))
        def newclasses = [mynewclass]
...
//                 def roi = new RectangleROI(100, 1000, 2000, 2000)
...
//             println(annotations)
//             addPathObjects(annotations) //TODO: can't add objects?
            fireHierarchyUpdate()
...
//         def annot = hierarchy.getAnnotationObjects()
//         println(annot)
// //         fireHierarchyUpdate()
//
//         hierarchy.getSelectionModel().setSelectedObject(annot[0])
//         runPlugin('qupath.imagej.detect.cells.WatershedCellDetection',
//                 '{"requestedPixelSizeMicrons": 0.25, "backgroundRadiusMicrons": 8, "medianRadiusMicrons": 0,  "sigmaMicrons": 1.5,  "minAreaMicrons": 10.0,  "maxAreaMicrons": 400.0,  "threshold": 0.1,  "watershedPostProcess": true,  "cellExpansionMicrons": 1.0,  "includeNuclei": true,  "smoothBoundaries": true,  "makeMeasurements": true}');
//
//         def det = hierarchy.getDetectionObjects()
//         println(det)
//
//         fireHierarchyUpdate()
