...
for case in case_list:
    seg_thumb_mask_img_arr = np.array(Image.open(os.path.join(seg_mask_thumb_dir, case, case+"_segmentation.png"), 'r'))
    thumb_img_mask_arr = np.array(Image.open(os.path.join(seg_mask_thumb_dir, case, case + "_thumbnail.png"), 'r'))
    entangle_area_mask = calculate_tumor_stroma_entangling_area(seg_thumb_mask_img_arr[:,:,0:3], seg_color_map)
    print("Locating relevant patches for case %s" % case)
    f_list = get_relevant_patches(entangle_area_mask, thumb_img_mask_arr, case, patch_root)
    all_img_patch_list += f_list
...
for idx, m in enumerate(all_class_list):
    ...
    for case in case_list:
        ...
        thumb_img_mask_arr = np.array(Image.open(os.path.join(seg_mask_thumb_dir, case, case + "_thumbnail.png"), 'r'))
        entangle_area_mask = calculate_tumor_stroma_entangling_area(seg_thumb_mask_img_arr[:,:,0:3], seg_color_map)
        print("Locating relevant patches")
        f_list = get_relevant_patches(entangle_area_mask, thumb_img_mask_arr, case, patch_root)

        save_to_dir = os.path.join(output_dir, case, m)
        if not os.path.exists(save_to_dir):
            if os.path.exists(os.path.join(patch_root, case)) :
